...
/*
 * FrontServlet이 넘겨준 요청 path를 가지고 실제 실행할 클래스의
 * 객체를 만들어서 리턴하는 작업을 수행한다.
 * => 미리 요청 path별로 실행할 실제 클래스를 등록할 설정파일을 보고 객체를 찾아서 생성
...
public class RequestMapping {
	...
	//요청 path가 key, 실제실행할 객체 value
	private HashMap<String,Action> actionMap = 
			new HashMap<String,Action>();
	public RequestMapping() {
		//생성자 내부에서 설정파일을 파싱해서
		//객체를 만드는 코드가 정의되어야 한다.
		
	}

	public Action mapping(String path) {
		//매개변수로 전달된 path로 등록된 Action객체를 actionMap에서 찾아서
		//리턴하는 코드가 정의되어야 하나 우리는 if...else if로 우선 처리
		//현재 코드는 연습용이므로 추가가 되면 mapping 메소드를 변경해야 하지만
